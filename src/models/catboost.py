...
def train_catboost_cv(
    ...
    model_version: str = "cat_v1",
    artifacts_dir: Path | str = Path("artifacts"),
) -> Tuple[np.ndarray, np.ndarray, Dict]:
    """
    Train CatBoost with stratified CV, returning OOF and test predictions.
    """
    if fold_cfg is None:
        fold_cfg = FoldConfig()
    ...
    X = train_with_folds[features].copy()
    y = train_with_folds[target_col].values
    X_test = test_df[features].copy()

    cat_indices: List[int] | None = None
    if categorical_features:
        # Map categorical feature names to column indices expected by CatBoost.
        cat_indices = [features.index(col) for col in categorical_features if col in features]

        # Defensive: ensure categorical columns are integer or string typed,
        # never floating, to satisfy CatBoost's requirements.
        cat_cols = [col for col in categorical_features if col in features]
        for col in cat_cols:
            if pd.api.types.is_float_dtype(X[col].dtype):
                # Cast float-encoded categories back to integers when safe.
